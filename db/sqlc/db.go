...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.createEntrieStmt, err = db.PrepareContext(ctx, createEntrie); err != nil {
		return nil, fmt.Errorf("error preparing query CreateEntrie: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.createEntrieStmt != nil {
		if cerr := q.createEntrieStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing createEntrieStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		createEntrieStmt:        q.createEntrieStmt,
